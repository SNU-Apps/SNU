...
public class CameraFragment extends Fragment
	...
	private void setUpCameraOutputs(int width, int height) {
		...
		try {
			for (String cameraId : manager.getCameraIdList()) {
				...
				// Resolution //was largest
				Size customSize = new Size(640, 480);
				mImageReader = ImageReader.newInstance(customSize.getWidth(), customSize.getHeight(),
	...
	private void captureStillPicture() {
		try {
			...
			/*
			byte 30 - 320x240 --> ~39 --> Qualitiy :|
			byte 50 - 320x240 --> ~39 --> Qualitiy :|
			byte 100 - 320x240 --> ~114 --> Qualitiy :) :)
			byte 20 - 640x480 --> ~45KB --> Qualitiy :)
			byte 30 - 640x480 --> ~49KB --> Qualitiy :) :)
			byte 30 - 800x600 --> ~106KB --> Qualitiy :) :)
			*/

			// Quality
			captureBuilder.set(CaptureRequest.JPEG_QUALITY, (byte) 30);
